...
var authenticate = function(apicall, opts, req, res, error_opts, callback){
  ...
  if(apicall.authentication){
    ...
    // if the token was found..look up the account data..
    if(token){
      ApiTokenService.getByToken(token, function(Error, ApiToken){
        if(!Error){
          // now make sure the account that is being accessed it valid for this user...
          if(opts.url_params.account_id){
            AccountService.get(opts.url_params.account_id, function(Error, Account){
              if(!Error){
                // now just make sure the Accounts match up...
                if(ApiToken.account_id == Account.account_id || ApiToken.account_id == Account.parent_account_id){
                  return callback(ApiToken);
                }else{
                  return returnError(req, res, error_opts, "InvalidPermissions");
                }
              }else{
                return invalidAuthetication(req, res, error_opts);  
              }
            });
          }else{
            return callback(ApiToken);
...
var delegate = function (apicall, opts, req, res, error_opts) {
  ...
  if(apicall.callback && apicall.library) {
    ...
    async.series(series, function(responseObj) {
      ...
      // get the request id then go ahead and return the results to the user...
      AppUtils.newId("request", function(request_id){
            res.header("X-Webhooksio-Request-Id", request_id);
            responseObj.request_id = request_id;
